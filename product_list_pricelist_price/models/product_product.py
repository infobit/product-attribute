...
class ProductProduct(models.Model):
    ...
    def _compute_product_pricelist_margin(self):
        pricelists = self.env["product.pricelist"].search(
            [("display_pricelist_price", "=", True)]
        )
        result = pricelists.price_rule_get_multi(
        ...
        for product in self:
            for pricelist in pricelists:
                field_name = "product_margin_pricelist_%s" % (pricelist.id)
                product[field_name] = (
                    ( 
    ...
    def _set_product_pricelist_price(self):
        pricelists = self.env["product.pricelist"].search(
            [("display_pricelist_price", "=", True)]
        )
        ...
        for product in self:
            for pricelist in pricelists:
                ...
                if product[field_name] != 0.0: # check value:
                    ...
                    if rule.applied_on in ['3_global','2_product_category','1_product'] or not rule:
                        values = {
                            'compute_price':'fixed',
    ...
    def _set_product_pricelist_margin(self):
        pricelists = self.env["product.pricelist"].search(
            [("display_pricelist_price", "=", True)]
        )
        # TODO: comprobar tipo de linea y modificar si es de producto 
    ...
    def _add_pricelist_margin(self, field_name, tag_name):
        self._add_field(
            ...
            fields.Float(
                string=tag_name, 
                compute="_compute_product_pricelist_margin",                
                inverse="_set_product_pricelist_margin"
            ),
        )
        return True
    ...
    def _register_hook(self):
        ...
        for pricelist in pricelists:
            ...
            field_margin_name = "product_margin_pricelist_%s" % (pricelist.id)
